...
export class ProiezioneController {
    ...
    update = async (req: Request, res: Response) => {
        const updatedProiezione = await this.proiezioneService.updateProiezione(res.locals.params.id, res.locals.body);
        res.status(200).json({ data: updatedProiezione });
    };

    delete = async (req: Request, res: Response) => {
        await this.proiezioneService.updateProiezioneExistence(res.locals.params.id, { eliminata: false });
        res.status(204).send();
    }
