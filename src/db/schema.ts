...
export const ruoloEnum = pgEnum('ruolo', ['ADMIN', 'USER']);
...
export const statoEnum = pgEnum('stato', ['CONFIRMED', 'PENDING', 'CANCELLED']);

// 7. Prenotazione
export const prenotazione = pgTable(
  'prenotazione',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    ...
    stato: statoEnum('stato').default('PENDING').notNull(),
    creata_il: timestamp('creata_il').notNull().defaultNow(),
    aggiornata_il: timestamp('aggiornata_il').notNull().defaultNow(),
    eliminata: boolean('eliminata').notNull().default(false),
  },
  (table) => ({
    // INDICE UNICO: Impedisce fisicamente a PostgreSQL di salvare 2 posti uguali per la stessa proiezione
    uniquePostoProiezione: uniqueIndex('unique_posto_proiezione_idx').on(
      table.proiezione_id,
      table.riga,
      table.colonna,
    ),
