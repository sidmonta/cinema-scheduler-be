...
export const classificazioneEnum = pgEnum('classificazione', ['T', '14+', '18+']);
...
export const sala = pgTable('sala', {
    id: uuid('id').defaultRandom().primaryKey(),
    cinema_id: uuid('cinema_id').notNull().references(() => cinema.id),
    nome: varchar('nome', { length: 255 }).notNull(),
    righe: integer('righe').notNull(),
    colonne: integer('colonne').notNull(),
    capienza: integer('capienza').generatedAlwaysAs(
        sql`"righe" * "colonne"`
    ),
    creata_il: timestamp('creata_il').notNull().defaultNow(),
    aggiornata_il: timestamp('aggiornata_il').notNull().defaultNow(),
...
export const proiezione = pgTable('proiezione', {
    id: uuid('id').defaultRandom().primaryKey(),
    sala_id: uuid('sala_id').notNull().references(() => sala.id),
    film_id: uuid('film_id').notNull().references(() => film.id),
...
export const prenotazione = pgTable('prenotazione', {
    id: uuid('id').defaultRandom().primaryKey(),
    utente_id: uuid('utente_id').notNull().references(() => utente.id),
    proiezione_id: uuid('proiezione_id').notNull().references(() => proiezione.id),
...
export const posto = pgTable('posto', {
    id: uuid('id').defaultRandom().primaryKey(),
    prenotazione_id: uuid('prenotazione_id').notNull().references(() => prenotazione.id),
